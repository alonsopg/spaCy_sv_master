...
f1 = open("./corpusxml2corenlp/bloggmix2017_training.txt", "a")
sentence = []
train_sentence = []
...
with open(sys.argv[1], "r") as f:
    for idx, line in enumerate(f):
        ...
        if (arr[1]=='LABEL\n'):
            ...
            cursor.execute("SELECT label FROM gazetters WHERE gazette='%s'" % arr[0])
            for row in cursor:
                arr[1] = row[0]+"\n"

            print(cursor.rowcount)
            if cursor.rowcount == 0 and arr[0][-1] == 's':
                cursor.execute("SELECT label FROM gazetters WHERE gazette='%s'" % arr[0][:-1])
                for row in cursor:
                    arr[1] = row[0]+"\n"

        if(old_arr[1] == 'LABEL\n' and arr[1] == 'LABEL\n'):
            cursor.execute("SELECT label FROM gazetters WHERE gazette='%s'" % arr[0] + ' ' + old_arr[0])
            for row in cursor:
                arr[1] = row[0]+"\n"
